...
카메라 위치, 수면 기준 오프셋, 보간 속도와 내부 적분 최대 스텝은 상위 요구에 없던 튜닝 기본값이다. 이 값들은 아래에서 근거와 검증 방법을 함께 정의한다.
...
이 seam은 `Source/ShipAutonomySim/Private` 안에 두고 `SHIPAUTONOMYSIM_API`로 export하지 않는다. `ShipMovement.cpp`와 같은 모듈의 자동화 테스트만 include한다. 테스트를 위해 `UShipMovement`의 public 함수나 상태 getter를 늘리지 않는다.
...
- possession과 unpossession 시 입력 mapping context 등록 및 해제
...
7. 이동 후보 XY에서 Ocean 표면 위치와 법선을 조회하고 Z와 up 방향을 보간한다.
...
채택 계수를 대입하면 `D = 399.9999999 cm`로 400 cm 목표와 일치한다.
...
- speed에 하드 최대값을 두지 않는다. 승인된 미분방정식의 안정 평형이 약 `+200 cm/s`와 `-200 cm/s`를 만든다.
- 수치 오류나 외부 상태 이상으로 평형 속도를 넘으면 저항이 다시 평형 방향으로 감속시킨다.
...
4. `travel_n = v_n * h`를 계산하고 `yaw_n`에서 만든 world 수평 전방 벡터에 곱해 후보 XY 이동을 만든다.
...
프레임 DeltaTime은 다음처럼 나눈다.

```text
N = max(1, ceil(DeltaTime / MaxSimulationStepSeconds))
h = DeltaTime / N
```

`DeltaTime`, `MaxSimulationStepSeconds`, `h`의 단위는 `s`이며 `N`은 무차원 양의 정수다.

모든 내부 스텝은 `h <= MaxSimulationStepSeconds`를 만족하며 정상적인 유한 DeltaTime 전체를 처리한다. 기본값은 `1/120 s`, 즉 약 `0.008333333 s`다.

이 기본값은 120 Hz를 기준 적분 해상도로 삼는다. 60 FPS에서는 2회, 30 FPS에서는 4회, 15 FPS에서는 8회 계산하므로 프레임 급락 시에도 같은 내부 해상도를 유지한다. 로컬 수치 검산에서 이 스텝의 forward Euler 결과는 다음과 같다.

- `180 cm/s` 최초 도달: 약 `3.992 s`
- `200 cm/s`에서 `5 cm/s`까지 타력 거리: 약 `399.96 cm`

`MaxSimulationStepSeconds`는 `UPROPERTY(EditAnywhere, Category=ShipMovement, meta=(ClampMin=0.001))` 후보로 둔다. 0 이하 또는 비유한 값이 들어오면 기본값으로 되돌리고 경고한다. 자동화 테스트는 15, 30, 60, 120 FPS 입력을 비교해 목표 시간과 거리의 프레임별 편차가 허용 범위 안인지 확인한다.

DeltaTime이 0 이하이면 해당 tick을 건너뛴다. DeltaTime이나 계산 결과가 비유한 값이면 transform을 적용하지 않고 speed를 0으로 만든 뒤 오류 상태를 표시한다.
...
3. 해당 내부 스텝의 후보 XY와 현재 Z로 world query 위치를 만든다.
...
5. 위치에는 `FWaterBodyQueryResult::GetWaterSurfaceLocation()`, 법선에는 `GetWaterSurfaceNormal()`을 사용한다. `IncludeWaves`가 있으므로 평면 값이 아니라 파도 변위를 포함한 표면 값이다.

### 성공 판정

다음 조건을 모두 만족할 때만 새로운 수면 sample을 채택한다.

- Water subsystem이 존재한다.
- Ocean body component weak pointer가 유효하다.
- 반환된 query flags에 `ComputeLocation`과 `ComputeNormal`이 있다. `IncludeWaves`는 항상 요청하되 Ocean에 wave data가 없어 엔진이 플래그를 조정한 경우에는 유효한 평면 surface location과 normal을 사용한다.
- exclusion volume 내부가 아니다.
- surface location과 normal이 유한하다.
- surface normal이 영벡터에 가깝지 않다.
...
- 화면 디버그에 `Water Valid` 또는 `Water Fallback` 상태를 표시한다.

## waterline offset과 수면 보간

`WaterlineOffsetCm`은 query한 surface location의 world Z에 더하는 값이다. 기본값은 `0 cm`다. 200 x 100 x 100 cm 대칭 box의 원점이 중앙이므로 기본값 0은 수면이 선체 중앙을 지나게 하는 중립적인 기준이다. 시각 메시를 바꾸지 않고 흘수만 조정할 수 있도록 `UPROPERTY(EditAnywhere, Category=ShipMovement)` 후보로 둔다.
...
두 기본값은 step 응답의 약 95퍼센트를 `0.6 s` 안에 따라가게 한다. 짧은 파도 변화의 시각적 떨림을 줄이면서 수면 변화에 1초 이내로 반응하는 검증용 절충값이다. 둘 다 양수 `UPROPERTY(EditAnywhere, Category=ShipMovement)` 후보로 두며 PIE에서 파도 마루 통과 시 선체가 튀거나 수면과 장시간 분리되지 않는지 확인한다.
...
수면의 pitch와 roll이 actor의 다음 수평 yaw 상태를 오염시키지 않도록 yaw는 별도 scalar state로 유지한다.

1. `FRotator(0, yaw_next, 0).Vector()`로 world 수평 전방을 만든다.
2. 이 전방을 보간된 surface normal 평면에 투영하고 normalize한다.
3. `FRotationMatrix::MakeFromXZ(TangentForward, SmoothedNormal).ToQuat()`로 X 전방과 Z up을 만족하는 회전을 만든다.
4. 투영 벡터가 퇴화하면 직전 유효 전방을 같은 평면에 투영하고, 그것도 실패하면 normal에 수직인 안전축을 사용한다.

이 방식은 수면 기울기를 적용하면서도 조향으로 적분한 수평 heading을 보존한다. actor의 기울어진 `GetActorRotation().Yaw`를 다음 스텝의 yaw 원본으로 다시 읽지 않는다.
...
세 값은 `UPROPERTY(EditAnywhere, Category=Camera)` 후보로 둔다. spring arm collision test는 켠다. PIE에서 200 cm 선체가 화면 밖으로 잘리지 않고 250 cm 반경 선회 방향을 관찰할 수 있는지 확인한다. 레벨에 기존 blocking geometry가 있는 경우에는 카메라 collision도 함께 확인하되 검증을 위해 벽 actor나 asset을 새로 추가하지 않는다.
...
세 멤버는 `UPROPERTY(Transient)`가 붙은 `TObjectPtr`로 보관하고 Pawn을 Outer로 사용한다. mapping별 `UInputModifierNegate`는 mapping context의 instanced modifier 배열이 소유한다.
...
로컬 `APlayerController`의 `ULocalPlayer`에서 `UEnhancedInputLocalPlayerSubsystem`을 구해 mapping context를 우선순위 0으로 등록한다. unpossession 시에는 controller 연결을 끊기 전에 같은 subsystem에서 등록했던 context를 제거해 중복 mapping을 남기지 않는다.
...
이 경로는 UE 5.5.4 기본 C++ 템플릿의 설정과 일치한다. 현재 `Build.cs`에는 `InputCore`와 `EnhancedInput`이 private dependency로 이미 있으므로 새 모듈은 필요하지 않다.

### 수동 입력과 이후 자동주행의 중재

3단계에서는 수동 모드만 활성화한다. 이후 자동주행 활성화 경계에는 다음 불변조건을 적용한다.

- 수동에서 자동으로 바뀌는 순간 Pawn이 `SetThrottle(0)`과 `SetSteer(0)`을 한 번 호출해 남은 키 값을 지운다.
...
`ShipPawnClass`와 `TestShipSpawnTransform`은 GameMode의 `UPROPERTY(EditAnywhere, Category=Stage3Test)` 후보로 둔다. class 기본값은 `AShipPawn`, transform 기본값은 world origin과 yaw 0이다. 원점은 별도 코스나 시작점 actor를 요구하지 않는 중립 기본값이며, MainLevel의 배치된 Ocean 위에서 첫 수면 query가 실제 Z를 결정한다. PIE에서 원점이 Ocean 유효 영역인지 확인하고, 레벨 조건이 바뀌면 이 property만 조정한다.
...
- yaw rate `deg/s`
- 이번 프레임 내부 substep 수와 step seconds
- water 상태, target surface Z, 적용 Z, waterline offset
- surface normal과 적용 up vector
- 마지막 blocking hit 대상과 impact normal
- 입력 모드 `Manual` 또는 이후 `Autopilot`

`bShowMovementDebug`는 기본 true인 `UPROPERTY(EditAnywhere, Category=Debug)` 후보로 두고 Shipping에서는 화면 출력을 하지 않는다. 디버그 표시는 관찰 전용이며 계산 상태를 수정하지 않는다.
...
| 음수 drag, 0 이하 기준 속도 또는 잘못된 최대 스텝 | 안전한 설계 기본값으로 복구하고 한 번 경고 |
| Water subsystem 또는 Ocean component 없음 | 마지막 유효 수면, 없으면 현재 Z와 world up 사용 |
| query location 또는 normal 무효 | 새 sample 폐기, water fallback 상태로 전환 |
...
- `Source/ShipAutonomySim/Public/ShipMovement.h`: 두 setter, component lifecycle 선언, 튜닝 `UPROPERTY`, UObject 포인터 전방 선언
- `Source/ShipAutonomySim/Private/ShipMovement.cpp`: tick, substep 조정, 수면 query, 보간, 회전 재구성, sweep, hit 및 debug 처리
- `Source/ShipAutonomySim/Private/ShipMovementSimulation.h`: module-private 값 타입과 순수 함수 선언
- `Source/ShipAutonomySim/Private/ShipMovementSimulation.cpp`: longitudinal 및 yaw 단일 스텝 계산
- `Source/ShipAutonomySim/Public/ShipPawn.h`: Pawn 구성, 입력 lifecycle override와 UObject 포인터 선언
- `Source/ShipAutonomySim/Private/ShipPawn.cpp`: box, cube, movement, camera 구성과 런타임 Enhanced Input adapter
...
- `Source/ShipAutonomySim/Private/Tests/ShipMovementTests.cpp`: pure motion step과 substep 자동화 테스트
- `Config/DefaultInput.ini`: Enhanced PlayerInput과 Enhanced InputComponent 기본 클래스
...
module-private `AdvanceShipMotion`을 직접 호출해 다음을 검증한다.

- throttle과 steer가 `[-1, 1]`에 머문다.
- 양수와 음수 속도에서 drag가 항상 속도 반대 방향이다.
- throttle 1의 평형 가속도가 `200 cm/s`에서 0에 가깝다.
- throttle -1의 평형 가속도가 `-200 cm/s`에서 0에 가깝다.
- 정지에서 `180 cm/s` 최초 도달 시간이 `4.0 s`에 대해 `0.05 s` 이내다.
- `200 cm/s` 타력 주행이 `5 cm/s`에 도달해 snap될 때 거리가 `400 cm`에 대해 `5 cm` 이내다.
...
- 15, 30, 60, 120 FPS DeltaTime을 내부 `1/120 s` 이하로 나눈 결과의 속도, 거리와 yaw 편차가 허용 범위 안이다.
- 비유한 입력과 잘못된 파라미터가 transform에 전달될 값으로 번지지 않는다.
...
- Input 설정이 `UEnhancedPlayerInput`과 `UEnhancedInputComponent`를 기본 클래스로 해석하는지 확인한다.
...
- 15, 30, 60, 120 FPS 제한에서 가속 시간, 정지 거리와 선회가 눈에 띄게 달라지지 않는다.
- 파도 위에서 Z와 pitch 및 roll이 부드럽게 따라가고 수평 yaw가 흔들리지 않는다.
- Ocean query를 사용할 수 없는 상태가 생기면 선박이 비유한 transform으로 사라지지 않고 fallback 표시가 나온다.
...
- 선형 및 이차 저항 계수와 정지 임계값이 자동화 테스트에서 정량 목표를 만족한다.
- 전진, 후진, 속도 비례 조향, 0속도 회전 금지가 동작한다.
- 최대 내부 스텝 제한으로 지원 FPS 범위의 결과가 허용 오차 안에 있다.
- Water query 성공과 실패 모두에서 유한한 transform을 유지한다.
- 수면 Z와 normal을 부드럽게 따라가면서 수평 yaw를 보존한다.
